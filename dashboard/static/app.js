...
async function api(url,options={}){
 ...
 const payload=await response.json().catch(()=>({status:'error',message:'The server returned an invalid response.'}));
 if(!response.ok){
  const requestId=response.headers.get('x-request-id')||payload.request_id;
  throw new Error((payload.message||'The request failed.')+(requestId?' Request ID: '+requestId:''));
 }
 return payload;
}
...
async function load(){
 ...
 document.querySelector('#news').innerHTML=d.news.length?d.news.map(x=>'<div class="item"><span class="tag">'+esc(x.ticker)+' · '+esc(x.sentiment||'NEWS')+'</span><a href="'+esc(x.article_url||'#')+'" target="_blank" rel="noopener noreferrer">'+esc(x.title)+'</a><small> · '+date(x.published_at)+'</small></div>').join(''):'<p class="muted">No synced articles yet.</p>';
 ...
 document.querySelectorAll('[data-remove]').forEach(button=>button.onclick=async()=>{
  try{
   await api('/api/watchlist/'+encodeURIComponent(button.dataset.remove),{method:'DELETE',headers:{'Idempotency-Key':writeKey()}});
   await load();
  }catch(error){window.alert(error.message);}
 });
}

document.querySelector('#ticker-form').onsubmit=async event=>{
 event.preventDefault();
 try{
  await api('/api/watchlist',{
   method:'POST',
   headers:{'Content-Type':'application/json','Idempotency-Key':writeKey()},
   body:JSON.stringify({ticker:document.querySelector('#ticker').value})
  });
  event.target.reset();
  await load();
 }catch(error){window.alert(error.message);}

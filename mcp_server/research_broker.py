...
def get_stock_performance(ticker: str, lookback_days: int = 30) -> dict:
    try:
        ...
        if not raw:
            raise ValueError(f"No daily price bars were found for {symbol}.")
        ...
        return {
            ...
            "period_high": max(x["high"] for x in bars),
            "period_low": min(x["low"] for x in bars),
            "daily_bars": bars,
        }
...
def get_watchlist(user_email: str, watchlist_name: str = "Primary") -> dict:
    try:
        ...
        watchlist_id = _watchlist(user_id, watchlist_name)
        rows = lakebase.query(
          ...
          WHERE wt.watchlist_id=%s ORDER BY wt.added_at""",
            (watchlist_id,),
        )
...
def update_watchlist(user_email: str, ticker: str, action: str, watchlist_name: str = "Primary") -> dict:
    try:
        ...
        result = get_watchlist(user_email, watchlist_name)
        result.update({"action": verb, "ticker": symbol})
        return result
    except Exception as error:
        return _error(error)
    ...
    try:
        ...
        row = lakebase.write(
          ...
          VALUES(%s,%s,%s,%s,%s) RETURNING id,created_at""",
            (_user(user_email), symbol, title.strip(), note_text.strip(), Json(thesis_tags or [])),
            True,
        )
...
def save_analysis_report(
    ...
    try:
        ...
        if not title.strip() or not report_text.strip():
        ...
        row = lakebase.write(
          ...
          VALUES(%s,%s,%s,%s,%s,%s) RETURNING id,created_at""",
            (_user(user_email), title.strip(), thesis, symbols, report_text.strip(), Json(source_context or {})),
            True,
        )
...
def get_notable_updates(user_email: str, move_threshold_percent: float = 5.0, mark_visited: bool = True) -> dict:
    try:
        user_id = _user(user_email)
        rows = lakebase.query(f"SELECT last_visit_at FROM {_table('users')} WHERE id=%s", (user_id,))
        ...
        news = lakebase.query(
          ...
          FROM {_table("news_articles")} n JOIN {_table("watchlist_tickers")} wt ON wt.ticker=n.ticker
          JOIN {_table("watchlists")} w ON w.id=wt.watchlist_id

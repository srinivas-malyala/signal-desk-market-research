...
def source_documents() -> list[dict]:
    companies = lakebase.table_name("companies")
    news_articles = lakebase.table_name("news_articles")
    rows = lakebase.query(f"""
...
def main(batch_size: int = 100) -> int:
    ...
    for source in documents:
        for index, text in enumerate(chunks(source["content"])):
            ...
            record_id = hashlib.sha256(
                f"{source['source_type']}:{source['source_id']}:{index}:{MODEL_NAME}".encode()
            ).hexdigest()
            records.append(
                {**source, "id": record_id, "chunk_index": index, "chunk_text": text, "content_hash": digest}
            )
    if not records:
    ...
    embeddings = model.encode(
        [row["chunk_text"] for row in records], batch_size=64, normalize_embeddings=True, show_progress_bar=True
    )
    connection = pg8000_connection()
    try:
        cursor = connection.cursor()
        sql = f"""INSERT INTO {lakebase.table_name("research_embeddings")}(id,source_type,source_id,ticker,chunk_index,chunk_text,content_hash,embedding,model_name)

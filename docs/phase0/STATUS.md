...
| 0.3 Databricks bundle foundation | Complete | Bundle targets verified schema `bootcamp_students.student_sri` and serverless warehouse `b15d3d6f837ba428`; managed jobs, pipelines, Volume, and AI Search resources have been deployed selectively; Databricks App resources are excluded; strict data-workspace validation and the separate two-service Render Blueprint contract pass | Remote CI execution only |
...
5/8 app runtimes are packaged for Render. The existing

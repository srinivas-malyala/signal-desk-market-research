...
|---|---|---|---|
| 2.1 Bronze market streaming tables | Complete locally | Modern Spark Declarative Pipelines API, Auto Loader, explicit response/manifest schemas, rescued data, immutable file/request lineage, and per-date reconciliation | Process the Volume pilot exactly once and prove every manifest row count matches Bronze |
...
The bundle validates against the selected `dataexpertio_srini` profile. A bundle deployment has not been performed because it would deploy the full application resource set, whose Lakebase credential is still pending. Once that runtime secret is available, run the 10-date pilot first, inspect reconciliation, and only then start the bounded full backfill.

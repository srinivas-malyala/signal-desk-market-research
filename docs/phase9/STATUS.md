...
|---|---|---|---|
| 9.1 Deployment | Render artifacts locally accepted; external deployment pending | Two-service Blueprint, `$PORT`/health packaging, data-bundle separation, OIDC/JWT identity, explicit M2M clients, architecture, cost boundary, risk register, rollback, and acceptance harnesses are implemented; 230 tests and Ruff pass | Renew Databricks OAuth, provision OIDC/M2M/Render secrets, strictly validate the data bundle, and deploy both services |

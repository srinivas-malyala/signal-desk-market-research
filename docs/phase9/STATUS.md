...
|---|---|---|---|
| 9.1 Deployment | Render artifacts and data-bundle validation accepted; external deployment pending | Two-service Blueprint, `$PORT`/health packaging, data-bundle separation, OIDC/JWT identity, explicit M2M clients, architecture, cost boundary, risk register, rollback, and acceptance harnesses are implemented; 231 tests and Ruff pass; strict data-only bundle validation passes with `dataexpertio_srini` | Provision OIDC/M2M/Render secrets and grants, then deploy both services |
